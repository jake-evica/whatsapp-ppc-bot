...
class PPCBidOptimizer:
    ...
    def save_results(self, output_file: str, optimized_df: pd.DataFrame):
        ...
        print(f"Optimized PPC bid data saved to: {output_file}")


if __name__ == "__main__":
    infile = "scripts/data/raw/sample_bid_0001.xlsx"
    outfile = 'scripts/data/processed/optimized_bids.xlsx'
    target_acos = 0.3  # 30% Target ACOS

    optimizer = PPCBidOptimizer(infile, target_acos)
    optimized_bids_df = optimizer.optimize_bids()
    optimizer.save_results(outfile, optimized_bids_df)
